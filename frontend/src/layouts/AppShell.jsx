...
export default function AppShell({ children, brand, brandSub, user, sidebarItems, headerExtra }) {
  ...
  const { notifications, clearNotifications, dismissNotification, showToast } = useNotifications();
  ...
  return (
    <div className={`app-shell ${isCinematicMode ? 'cinematic-active' : ''}`}>
      ...
      {/* Mobile Overlay */}
      {isSidebarOpen && (
        <div className="modal-overlay" style={{ zIndex: 10000 }} onClick={() => setIsSidebarOpen(false)} />
      )}
      ...
      <main className="main-content">
        <header className="top-header">
          <div className="header-left">
            <button className="btn-icon mobile-only" onClick={() => setIsSidebarOpen(!isSidebarOpen)} style={{ marginRight: 12 }}>
              ...
              {activeDropdown === 'notif' && (
                <div className="header-dropdown">
                  <div className="dropdown-header">
                    ...
                    <button className="btn-ghost" style={{ fontSize: '0.6rem' }} onClick={clearNotifications}>Clear All</button>
                  </div>
